...
def create_router(commercial: CommercialService, pcp: PcpService, coordination: CoordinationService) -> APIRouter:
    ...
    @router.get("")
    def search(request: Request, q: str = Query(min_length=2, max_length=100)) -> list[dict[str, str]]:
        term = q.casefold()
        result: list[dict[str, str]] = []
        user = getattr(request.state, "user", None)
        permissions = None if user is None or user.role.value == "administrador" else user.permissions
        role = user.role.value if user else "administrador"
        quote_access = permissions is None and role in {"administrador", "comercial"} or permissions is not None and bool({"quotes", "crm"} & set(permissions))
        pcp_access = permissions is None and role in {"administrador", "pcp", "producao", "analista_custos"} or permissions is not None and "pcp" in permissions
        coordination_access = permissions is None and role in {"administrador", "comercial", "pcp", "engenharia", "producao", "qualidade"} or permissions is not None and "coordination" in permissions
        if quote_access:
